...
export async function getServerBranding() {
  ...
  try {
    ...
    // Pre-login (no authenticated user) always uses the build's base theme
    // (e.g. KYRGA blue), never a per-company palette override.
    const palette = user ? (company?.palette || DEFAULT_PALETTE) : DEFAULT_PALETTE;
