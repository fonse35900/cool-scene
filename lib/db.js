...
async function initDb(c) {
  ...
  if (!companyRow.rows[0]) {
    ...
    await c.execute({
      sql: "INSERT INTO companies (id, name, logo, branding_configured) VALUES (1, ?, ?, 1)",
      args: [s.company_name || BRAND.name, s.logo || BRAND.logo],
    });
  }

  // Migration: add branding_configured to companies (existing companies are considered configured)
  const companyCols = await c.execute("PRAGMA table_info(companies)");
  if (!companyCols.rows.some(r => r.name === 'branding_configured')) {
    await c.execute("ALTER TABLE companies ADD COLUMN branding_configured INTEGER DEFAULT 0");
    await c.execute("UPDATE companies SET branding_configured = 1");
  }
  if (!companyCols.rows.some(r => r.name === 'suspended')) {
    await c.execute("ALTER TABLE companies ADD COLUMN suspended INTEGER DEFAULT 0");
  }
  if (!companyCols.rows.some(r => r.name === 'palette')) {
    await c.execute("ALTER TABLE companies ADD COLUMN palette TEXT DEFAULT 'octane'");
  }
  ...
  // Financial dates: explicit purchase and sale dates on vehicles
  if (!vehCols.rows.some(r => r.name === 'purchase_date')) {
    await c.execute("ALTER TABLE vehicles ADD COLUMN purchase_date TEXT");
    // Backfill from existing timestamps so past data has sensible dates
    await c.execute("UPDATE vehicles SET purchase_date = substr(created_at, 1, 10) WHERE purchase_date IS NULL");
  }
  if (!vehCols.rows.some(r => r.name === 'sale_date')) {
    await c.execute("ALTER TABLE vehicles ADD COLUMN sale_date TEXT");
    await c.execute("UPDATE vehicles SET sale_date = substr(updated_at, 1, 10) WHERE sale_date IS NULL AND status = 'vendido'");
  }
