...
export function BrandingProvider({ children, initial }) {
  // Seed from the server-resolved branding so the first paint already has the
  // correct logo/name/palette (no flash of the default brand on navigation).
  const [branding, setBranding] = useState(initial || { name: BRAND.name, logo: BRAND.logo, tagline: BRAND.tagline, configured: true, palette: DEFAULT_PALETTE });
